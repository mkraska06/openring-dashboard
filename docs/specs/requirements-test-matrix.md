...
Last checked: 2026-07-28 against the current Flutter implementation.
...
| VM-04 | Auto-connect to saved ring on app startup | Integration + Manual | Partial | `ScanPageNotifier` auto-connects to the saved device ID and [test/ui/scan_page_controller_test.dart](../../test/ui/scan_page_controller_test.dart) covers the fake BLE flow; add manual startup acceptance test with a real ring. |
...
| HF-03 | Display selected day's heart-rate history graphically | Storage / Unit / Widget | Covered | Storage day filtering, chart models, and a history widget smoke test with sample HR data are covered by [test/storage/storage_repository_test.dart](../../test/storage/storage_repository_test.dart), [test/history/history_chart_models_test.dart](../../test/history/history_chart_models_test.dart), and [test/widget_test.dart](../../test/widget_test.dart). |
...
| ID | Requirement Summary | Test Type | Status | Evidence / Suggested Test |
| --- | --- | --- | --- | --- |
| AC-01 | Show current accelerometer X/Y/Z raw values | Unit / Widget | Covered | Parser tests cover signed axes and [test/widget_test.dart](../../test/widget_test.dart) covers `AccelerometerCard` rendering diagnostics and values. |
...
| AC-05 | Provide per-session statistics | Unit | Covered | [test/motion/gesture_motion_analysis_test.dart](../../test/motion/gesture_motion_analysis_test.dart) covers statistics, stability, and grouping behavior. |
...
| AC-07 | Classify held hand positions from measured centers | Unit | Covered | [test/gesture_hub/gesture_hub_controller_test.dart](../../test/gesture_hub/gesture_hub_controller_test.dart) covers classification of measured open-hand and fist positions. |
...
| DE-03 | Export selected data as CSV | Unit / Integration | Partial | CSV formatter, Drift export repository, and selected-folder file writing are covered by [test/data_export/export_formatter_test.dart](../../test/data_export/export_formatter_test.dart), [test/data_export/export_repository_test.dart](../../test/data_export/export_repository_test.dart), and [test/data_export/export_controller_test.dart](../../test/data_export/export_controller_test.dart); native desktop folder picker needs manual validation. |
| DE-04 | Export selected data as JSON | Unit / Integration | Partial | JSON formatter and Drift export repository are covered by [test/data_export/export_formatter_test.dart](../../test/data_export/export_formatter_test.dart) and [test/data_export/export_repository_test.dart](../../test/data_export/export_repository_test.dart); export cancellation and the shared selected-folder write flow are covered by [test/data_export/export_controller_test.dart](../../test/data_export/export_controller_test.dart), native desktop folder picker needs manual validation. |
| DE-05 | User can choose which data types to export | Widget / Integration | Covered | Export UI filter interaction and repository type filtering are covered by [test/data_export/export_card_test.dart](../../test/data_export/export_card_test.dart) and [test/data_export/export_repository_test.dart](../../test/data_export/export_repository_test.dart). |
...
| OV-02 | Overlay shows current heart rate in BPM | Widget | Covered | [test/overlay/overlay_widget_test.dart](../../test/overlay/overlay_widget_test.dart) covers rendering the heart-rate value. |
| OV-03 | Overlay shows current SpO2 percentage | Widget | Covered | [test/overlay/overlay_widget_test.dart](../../test/overlay/overlay_widget_test.dart) covers rendering the SpO2 value. |
| OV-04 | Overlay shows battery percentage | Widget | Covered | [test/overlay/overlay_widget_test.dart](../../test/overlay/overlay_widget_test.dart) covers rendering the battery value. |
| OV-05 | Overlay shows current day's steps | Widget | Covered | [test/overlay/overlay_widget_test.dart](../../test/overlay/overlay_widget_test.dart) covers rendering daily activity steps. |
| OV-06 | Overlay renders BPM values over 120 in red | Widget | Covered | [test/overlay/overlay_widget_test.dart](../../test/overlay/overlay_widget_test.dart) covers high heart-rate threshold styling. |
| OV-07 | Overlay renders SpO2 values under 95% in red | Widget | Covered | [test/overlay/overlay_widget_test.dart](../../test/overlay/overlay_widget_test.dart) covers low SpO2 threshold styling. |
| OV-08 | User can configure thresholds | Storage / Widget | Partial | Threshold settings persist and are tested in [test/overlay/overlay_widget_test.dart](../../test/overlay/overlay_widget_test.dart); visible settings UI is still missing. |
...
| OV-10 | Persist overlay position | Storage / Integration | Covered | [test/overlay/overlay_widget_test.dart](../../test/overlay/overlay_widget_test.dart) covers `OverlaySettingsNotifier` position persistence through `SharedPreferences`. |
...
| ID | Requirement Summary | Test Type | Status | Evidence / Suggested Test |
| --- | --- | --- | --- | --- |
| NF-01 | Targets Windows 11 as primary desktop platform | Manual / Review | Covered | The app has a Windows runner and native Windows integrations for overlay, tray, hotkeys, and Gesture Hub controls. The Windows desktop app was built and manually started successfully; installer/release distribution is outside this requirement. |
| NF-02 | Keeps codebase portable enough for Linux desktop builds where dependencies are available | Review / Manual | Partial | A Linux runner exists and most app logic is Dart/Flutter; BLE setup, native window behavior, and input-service parity need manual Linux validation. |
| NF-03 | Stores user data locally by default | Storage / Architecture | Covered | Drift/SQLite storage is used for devices, settings, vitals, battery snapshots, activity intervals, and motion sessions; storage behavior is covered by [test/storage/storage_repository_test.dart](../../test/storage/storage_repository_test.dart). |
| NF-04 | Does not require a cloud account or backend service for core functionality | Architecture / Review | Covered | `pubspec.yaml` contains no backend/cloud account dependency, and the documented architecture is local-first. |
| NF-05 | Keeps protocol parsing and packet generation deterministic and covered by automated tests | Unit | Covered | [test/protocol/](../../test/protocol/) covers packet checksums, command builders, and parsers for battery, real-time values, HR logs, HR settings, steps/activity, and accelerometer data. |
| NF-06 | Isolates hardware-dependent behavior behind service or controller boundaries where practical | Architecture / Review | Partial | BLE, overlay/window behavior, and Gesture Hub native input paths are separated behind services/controllers; real platform behavior still needs manual validation. |
